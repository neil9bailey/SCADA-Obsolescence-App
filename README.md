...
- TPCMS workbook `.xlsx` import plus platform CSV import, with row validation and upsert
- CSV export of the current relational register and source-field export for TPCMS evidence
...
## Workbook and CSV migration

Use the **Data exchange** page or POST a TPCMS workbook `.xlsx` or UTF-8 CSV to `/api/assets/import`.

For the supplied live workbook, the importer reads the `Obsolescence Register` sheet, maps
each source row into the platform asset model, preserves all source columns in structured
metadata, retains workbook formula text, and adds source intelligence for filtering and
reporting. Re-importing the same workbook updates the same generated TPCMS asset codes.
...
The sample file at `data/sample-import.csv` demonstrates the platform CSV structure. See
`docs/DATA_DICTIONARY.md` for field definitions and validation rules. Existing `asset_code`
values are updated. `programme_code` can be used to link an asset to an existing package.
Calculated fields such as `risk_score` are ignored and regenerated by the backend.
...
- `/api/assets/source-export.csv`: original source workbook columns with platform risk,
  wave, category, automation flag and formula-column evidence.
...
The tests cover health, dashboard rollups, asset CRUD and recalculation, programme linkage,
funding aggregation, row-isolated CSV import/export, TPCMS workbook import, source metadata
retention, source dashboard summary and source-field export.
